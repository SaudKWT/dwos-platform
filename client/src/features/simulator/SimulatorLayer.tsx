  ...
  useEffect(() => {
    for (const loc of Object.values(ctx.locsById)) {
      ...
      const c = L.circle([loc.lat, loc.lon], {
        radius: 500,
        color: '#ff7849', weight: 1, opacity: 0.45, dashArray: '4,5',
        fillColor: '#ff7849', fillOpacity: 0.05, interactive: false,
      }).addTo(map)
    ...
    for (const vid in timelines) {
      ...
      for (const s of timelines[vid].filter(s => s.type === 'transit')) {
        ...
        const p = L.polyline(path, {
          color: v.color ?? '#888', weight: 1.5, opacity: 0.35, dashArray: '3,5',
        }).addTo(map)
      ...
      for (const vid in aisTracksByVid) {
        ...
        if (track.length >= 2) {
          const line = L.polyline(track.map(p => [p.lat, p.lon] as [number, number]), {
            color: v.color ?? '#888', weight: 3, opacity: 0.85, lineCap: 'round',
          }).addTo(map)
        ...
        for (const p of track) {
          const dot = L.circleMarker([p.lat, p.lon], {
            radius: 3, color: v.stroke ?? '#222', weight: 1,
            fillColor: v.color ?? '#888', fillOpacity: 1,
          }).addTo(map)
          dot.bindTooltip(
    ...
    const buildMarkers = () => {
      ...
      for (const v of vessels) {
        ...
        const svg = isCrewBoat
          ? fastCrewSvg(v.color ?? '#888', v.stroke ?? '#333')
          : psvSvg(v.color ?? '#888', v.stroke ?? '#333')
    ...
    const frame = () => {
      ...
      for (const v of vessels) {
        ...
        const line = L.polyline(covered, {
          color: v.color ?? '#888', weight: 3, opacity: 0.85,

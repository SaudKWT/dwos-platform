// Vessel and rig marker artwork, as inline SVG data URIs.
//
// The hexes below are ILLUSTRATION, not theme, and are the deliberate exception
// to "never hand-write a hex": they are the pictorial content of an icon in the
// same way a logo's colours are. A PSV's deck is buff, its navigation lights are
// yellow, the jack-up rig's legs are dark — those do not become blue because the
// dashboard is in dark mode, any more than a hazard sign does.
//
// What IS theme-driven is passed in: the hull fill and stroke come from
// dbo.Vessel.MapColor / .MapStroke via the caller. Anything that should follow
// the KOC tokens belongs in ./mapTokens.ts instead.

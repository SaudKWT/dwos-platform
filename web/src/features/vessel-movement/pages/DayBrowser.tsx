...
import { useReport, useReportIndex, useVessels } from '@/features/vessel-movement/api/queries'
import { openReportPdf } from '@/features/vessel-movement/report-form/buildReportHtml'
import type { ReportIndexRow } from '@/features/vessel-movement/api/types'
...
/**
 * `focus` lets the All-reports table hand a specific report to the reader.
 * Without it the "Open" button could only switch tabs and dump you on the
 * newest day, which is the one place you already were.
 */
export interface DayBrowserFocus {
  date: string
  vesselId: string
  /** Bumped by the caller so re-opening the same report re-focuses it. */
  nonce: number
}

export default function DayBrowser({ focus }: { focus?: DayBrowserFocus }) {
  ...
  // A report handed over from the table wins over whatever was open.
  useEffect(() => {
    if (focus && days.length) openDay(focus.date, focus.vesselId)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [focus?.nonce, days.length])
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        {day && (
          <div className="flex flex-wrap items-center gap-2 border-b bg-card px-6 py-2.5">
            ...
            <Link
              to={`/unit-4/vessels/map?t=${day.date}T12:00`}
